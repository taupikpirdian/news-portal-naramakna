...
@if($shouldDisplay)
    @if($isLocalhost && $dataSource === 'static' && $publisherId && strpos($publisherId, 'ca-pub-') === 0)
        ...
        @php
            $isSidebar = in_array($type, ['sidebar_left', 'sidebar_right']);
            $padding = $isSidebar ? '0.75rem' : '1rem';
            $iconSize = $isSidebar ? '28px' : '40px';
            $titleSize = $isSidebar ? '0.875rem' : '1rem';
            $textSize = $isSidebar ? '0.65rem' : '0.75rem';
            $infoBoxPadding = $isSidebar ? '0.5rem 0.6rem' : '0.6rem 0.8rem';
        @endphp

        <div class="google-ads-container google-ads-{{ $type }} {{ $attributes->class ?? 'my-4' }}"
             style="{{ $isSidebar ? 'width: 160px;' : 'width: 100%; max-width: 100%;' }} background: #facc15; border-radius: 8px; padding: {{ $padding }}; text-align: center; position: relative; overflow: hidden;">

            {{-- Content --}}
            <div style="position: relative; z-index: 1; color: #1f2937;">
                {{-- Icon --}}
                <div style="margin-bottom: {{ $isSidebar ? '0.4rem' : '0.6rem' }};">
                    <svg style="width: {{ $iconSize }}; height: {{ $iconSize }}; margin: 0 auto; opacity: 0.9;" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/>
                    </svg>
                </div>

                {{-- Main Text --}}
                <h3 style="font-size: {{ $titleSize }}; font-weight: 700; margin: 0 0 {{ $isSidebar ? '0.25rem' : '0.4rem' }}; line-height: 1.2;">
                    📢 {{ $isSidebar ? 'Ads' : 'Advertisement Space' }}
                </h3>

                {{-- Subtitle --}}
                @if(!$isSidebar)
                <p style="font-size: 0.8rem; margin: 0 0 0.75rem 0; opacity: 0.9;">
                    <span style="font-weight: 600;">{{ str_replace('_', ' ', ucfirst($type)) }}</span> Ad Unit
                </p>
                @endif

                {{-- Info Box --}}
                <div style="background: rgba(255,255,255,0.5); backdrop-filter: blur(10px); border-radius: 6px; padding: {{ $infoBoxPadding }}; margin: {{ $isSidebar ? '0.4rem 0' : '0 auto' }}; {{ $isSidebar ? 'width: 100%;' : 'max-width: 400px;' }} border: 1px solid rgba(255,255,255,0.5);">
                    <p style="font-size: {{ $textSize }}; margin: 0; line-height: {{ $isSidebar ? '1.3' : '1.5' }};">
                        <span style="opacity: 0.95;">⚠️ <strong>Dev Mode</strong>{{ $isSidebar ? '' : ': Google AdSense does not work on localhost' }}</span>
                    </p>
                    <p style="font-size: {{ $textSize }}; margin: {{ $isSidebar ? '0.2rem' : '0.4rem' }} 0 0 0; line-height: {{ $isSidebar ? '1.3' : '1.5' }}; opacity: 0.95;">
                        ✅ {{ $isSidebar ? 'Production' : 'Ads will appear on' }} <strong>{{ $isSidebar ? 'only' : 'production domain' }}</strong>
                    </p>
                </div>

                {{-- Size Badge --}}
                <div style="margin-top: {{ $isSidebar ? 'auto' : '0.75rem' }}; display: inline-flex; align-items: center; gap: {{ $isSidebar ? '0.25rem' : '0.4rem' }}; background: rgba(255,255,255,0.5); padding: {{ $isSidebar ? '0.2rem 0.5rem' : '0.3rem 0.6rem' }}; border-radius: 20px; font-size: {{ $textSize }};">
                    <span style="opacity: 0.95;">📏</span>
                    <span style="opacity: 1;">{{ $isSidebar ? '160px' : $dimensions['width'] . ' × ' . $dimensions['height'] }}</span>
                </div>
            </div>
    ...
    @elseif($dataSource === 'static' && $publisherId && strpos($publisherId, 'ca-pub-') === 0)
        ...
        <div class="google-ads-container google-ads-{{ $type }} {{ $attributes->class ?? '' }}" style="{{ in_array($type, ['sidebar_left', 'sidebar_right']) ? 'width: 160px;' : 'width: 100%; max-width: 100%;' }}">
            ...
            <ins id="{{ $uniqueId }}"
                 class="adsbygoogle"
                 style="display:block; {{ in_array($type, ['sidebar_left', 'sidebar_right']) ? 'min-width:160px; width:160px;' : 'min-width:250px;' }} {{ in_array($type, ['sidebar_left', 'sidebar_right']) ? 'min-height:250px; max-height:600px;' : 'min-height:90px;' }}"
            ...
            <script>
                (function() {
                    ...
                    if (!adSlot) {
                        logDebug('warning', 'Ad slot is empty! Ads may not display properly.', {
                            adType: adType,
                            message: 'Please configure ADS_' + adType.toUpperCase() + '_SLOT in .env file'
                        });
                    }
