...
<div class="grid grid-cols-12 gap-8">
    ...
    <div class="sticky top-[140px] flex flex-col items-center gap-2">
        ...
        <a id="sVerticalTwitter" href="https://twitter.com/intent/tweet?url={{ urlencode(request()->url()) }}&text={{ urlencode($post['title']) }}" target="_blank" title="Bagikan di Twitter" aria-label="Bagikan di Twitter" class="w-9 h-9 rounded-full bg-[#1DA1F2] text-white flex items-center justify-center no-underline">
        <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24">
            <path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"/>
        </svg>
        </a>
        <a id="sVerticalWhatsApp" href="https://api.whatsapp.com/send/?phone=628979132802&text={{ urlencode($post['title']) }}%20{{ urlencode(request()->url()) }}&type=phone_number&app_absent=0" target="_blank" title="Bagikan di WhatsApp" aria-label="Bagikan di WhatsApp" class="w-9 h-9 rounded-full bg-[#25D366] text-white flex items-center justify-center no-underline">
        <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24">
        ...
        <a id="sVerticalFacebook" href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(request()->url()) }}" target="_blank" title="Bagikan di Facebook" aria-label="Bagikan di Facebook" class="w-9 h-9 rounded-full bg-[#1877F2] text-white flex items-center justify-center no-underline">
        <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24">
    ...
    <header class="mb-6">
        <a href="{{ route('category', ['slug' => $categorySlug]) }}" class="inline-block px-3 py-1.5 bg-yellow-450 text-white text-xs font-semibold rounded-full no-underline">{{ $category }}</a>
        <h1 itemprop="headline" class="text-3xl sm:text-4xl font-bold text-gray-900 mt-4 leading-tight">{{ $post['title'] }}</h1>
        <div class="flex flex-wrap gap-3 items-center text-gray-600 text-sm mt-3">
        <span itemprop="author">{{ $authorName }}</span>
        <span class="w-1 h-1 bg-gray-300 rounded-full"></span>
        <time itemprop="datePublished" datetime="{{ $date->toIso8601String() }}">{{ $date->format('d/m, H.i') }}</time>
        <span class="w-1 h-1 bg-gray-300 rounded-full"></span>
        <span>{{ $category }}</span>
        </div>
    </header>
    ...
    <figure class="mb-6">
        <img src="{{ $imageUrl }}" alt="{{ $post['title'] }}" class="w-full rounded-xl object-cover max-h-[520px]" onerror="this.onerror=null;this.src='{{ $imageUrl }}';">
    ...
    <div class="flex flex-wrap items-center gap-2 mt-8">
        ...
        <a href="#" class="px-3 py-1.5 text-xs font-medium rounded-full no-underline bg-gray-100 text-gray-700">{{ $tag }}</a>
        @endforeach
    </div>
    @endif
    ...
    <div class="mt-10 bg-gray-50 border border-gray-200 rounded-2xl p-8 text-center">
        ...
        <a href="https://www.instagram.com/naramakna.id?igsh=ejNla2VjeDdwaWd5" target="_blank" rel="noopener noreferrer" aria-label="Instagram" class="no-underline hover:text-yellow-450 transition-colors">
        ...
        <a href="https://www.facebook.com/naramakna.id/?_rdr" target="_blank" rel="noopener noreferrer" aria-label="Facebook" class="no-underline hover:text-yellow-450 transition-colors">
        ...
        <a href="https://x.com/apcomsolutions?s=21" target="_blank" rel="noopener noreferrer" aria-label="Twitter" class="no-underline hover:text-yellow-450 transition-colors">
    ...
    <div class="bg-white rounded-2xl shadow-sm p-4 mb-6">
        ...
        @foreach($latestPosts as $lPost)
            @php
                ...
                $lSlug = $lPost['slug'] ?? '#';
            @endphp
            <a href="{{ route('detail', ['slug' => $lSlug]) }}" class="flex gap-3 no-underline rounded-xl p-2 hover:bg-gray-50">
            <img src="{{ $lImg }}" alt="{{ $lPost['title'] }}" class="w-16 h-16 object-cover rounded-lg" onerror="this.onerror=null;this.src='{{ $lImg }}';">
            <div class="flex-1">
            <div class="text-sm font-semibold text-gray-800 leading-snug line-clamp-2">{{ $lPost['title'] }}</div>
...
<section class="mt-10 mb-6">
    ...
    @foreach($relatedPosts as $rPost)
        @php
            ...
            $rSlug = $rPost['slug'] ?? '#';
        @endphp
        <a href="{{ route('detail', ['slug' => $rSlug]) }}" class="bg-white rounded-xl overflow-hidden no-underline transition-all shadow-sm hover:shadow-xl hover:-translate-y-1 group">
        <div class="relative">
        <img src="{{ $rImg }}" alt="{{ $rPost['title'] }}" class="w-full h-[200px] object-cover" onerror="this.onerror=null;this.src='{{ $rImg }}';">
        <span class="absolute top-3 left-3 px-2.5 py-1 bg-yellow-450 text-white text-xs font-semibold rounded-full">{{ $rCategory }}</span>
        </div>
        <div class="p-4">
        <h4 class="text-base font-semibold text-gray-800 mb-2 leading-snug line-clamp-2 group-hover:text-yellow-450">{{ $rPost['title'] }}</h4>
